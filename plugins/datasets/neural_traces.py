...
class NeuralTracesDataset(ClusteringDataset):
    ...
    def observable_to_artifact(self, obs: Array) -> tuple[Array, ArtifactType]:
        ...
        Args:
            obs: Neural trace array of shape (data_dim,)

        Returns:
            Tuple of (reformatted array for visualization, type indicator)
        """
        # For now, just return the chirp response as an image/plot
        chirp_response = obs[: self.chirp_len]
        return chirp_response, ArtifactType.SERIES
