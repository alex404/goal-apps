...
class KMeansModel(ClusteringModel, CanComputePrototypes):
    ...
    def train(
        ...
        log.info(
            f"Fitting K-means with {self._n_clusters} clusters on {train_data.shape} data"
        )
        ...
        # Run analyses immediately (since sklearn state isn't saved)
        log.info("Running K-means analyses")
        params = jnp.array([0.0])
        for analysis in self.get_analyses(dataset):
            analysis.process(key, handler, logger, dataset, self, 0, params)
    ...
    def compute_cluster_prototypes(self, params: Array) -> list[Array]:
        """Compute cluster prototypes (centers) from the trained model."""
        if self._cluster_centers is None:
    ...
    def analyze(
        ...
        """Run analysis suite for K-means."""
        log.info("Running K-means analyses")
        # Load dummy params (K-means stores state internally)
        params = handler.load_params()
        epoch = handler.resolve_epoch or 0
        for analysis in self.get_analyses(dataset):
            analysis.process(key, handler, logger, dataset, self, epoch, params)
    ...
    def get_analyses(
        ...
        return [ClusterStatisticsAnalysis()]
...
class PCAKMeansModel(ClusteringModel, CanComputePrototypes):
    ...
    def train(
        ...
        self._pipeline = Pipeline([("pca", self._pca), ("kmeans", self._kmeans)])

        log.info(
            f"Fitting PCA({self.n_components}) + K-means({self._n_clusters}) on {train_data.shape} data"
        )
        ...
        # Run analyses immediately (since sklearn state isn't saved)
        log.info("Running PCA + K-means analyses")
        params = jnp.array([0.0])
        for analysis in self.get_analyses(dataset):
            analysis.process(key, handler, logger, dataset, self, 0, params)
    ...
    def analyze(
        ...
        """Run analysis suite for PCA + K-means."""
        log.info("Running PCA + K-means analyses")
        # Load dummy params (pipeline stores state internally)
        params = handler.load_params()
        epoch = handler.resolve_epoch or 0
        for analysis in self.get_analyses(dataset):
            analysis.process(key, handler, logger, dataset, self, epoch, params)
