...
class LDAModel(ClusteringModel):
    ...
    def train(
        ...
        log.info(f"Fitting LDA with {self._n_clusters} topics on {train_data.shape} data")
        ...
        if dataset.has_labels:
            ...
            logger.log_metrics({"LDA/Train Accuracy": (20, jnp.array(accuracy))}, jnp.array(0))
            ...
            logger.log_metrics({"LDA/Test NMI": (20, jnp.array(test_nmi))}, jnp.array(0))
            logger.log_metrics({"LDA/Test ARI": (20, jnp.array(test_ari))}, jnp.array(0))
            logger.log_metrics({"LDA/Test Accuracy": (20, jnp.array(test_accuracy))}, jnp.array(0))
            
            log.info(f"LDA Train NMI: {nmi:.3f}, ARI: {ari:.3f}, Accuracy: {accuracy:.3f}")
            log.info(f"LDA Test NMI: {test_nmi:.3f}, ARI: {test_ari:.3f}, Accuracy: {test_accuracy:.3f}")
        
        # Log perplexity (lower is better for LDA)
        perplexity = self._lda.perplexity(train_data)
        logger.log_metrics({"LDA/Train Perplexity": (20, jnp.array(perplexity))}, jnp.array(0))
    ...
    def generate(self, params: Array, key: Array, n_samples: int) -> Array:
        ...
        topic_proportions = rng.dirichlet(np.ones(self._n_clusters) * self.alpha, n_samples)
